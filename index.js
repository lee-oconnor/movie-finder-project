            ...
            searchedMovies.innerHTML += `
             <div class="found-movie" id="${filmImdbID}">
                 <div class="movie-poster">
                     <img src="${
                       dataObj.filmPoster
                     }" alt="Movie Poster" id="filmPoster-${i}">
                 </div>
                 <div class="movie-details">
                   <h4 class="movie-title" id="filmTitle-${i}">${
              dataObj.filmTitle
            } <span id="filmRating-${i}" class="film-rating"><img id="star" src="./img/star.png"alt="star rating">${
              dataObj.filmRating
            }</span>
                   </h4>
                    <span class="min-genre-watchlist">
                        <span id="runtime-${i}" class="runtime">${
              dataObj.filmRuntime
            }</span>
                        <span id="genre-${i}" class="genre">${
              dataObj.filmGenre
            }</span> 
                         <button class="watchlist-btn" add-state=${
                           localStorage.getItem(filmImdbID)
                             ? "removable"
                             : "addable"
                         } onClick="addToWatchlist(event)">Watchlist</button>
                     </span>
                     <div class="movie-plot" id="filmPlot-${i}">${
              dataObj.filmPlot
            }</div>
                 </div>
             </div>
